...
import { createAPostCommentAction, createACommentOnCommentAction } from "features/comments";
import { selectComment } from './../../features/comments/comment.selectors';
import { createACommentInitials } from './../../schemas/initialFormsValue/comment.initialValues';
...
interface Props {
  ...
  currentPostORComment: any;
  postId: string;
  isForPost: boolean;
  commentId?: string
}

const AddReplies: React.FC<Props> = ({ 
  isModalOpen,
  closeModal,
  value,
  title,
  handleClick,
  currentPostORComment,
  postId,
  isForPost,
  commentId,
}) => {
...
/*   useEffect(() => {
  ...
  const {
    ...
    defaultValues: {
      ...createACommentInitials,
    },
    resolver: yupResolver(createPostSchemaValidator),
  });

  const onSubmit: SubmitHandler<CreatePostTypes> = async (data: CreatePostTypes) => {    
    if (postId && commentId) {
      await dispatch(
        createACommentOnCommentAction({
          replyInfo: {...data, isBaseComment: false},
          postId: postId,
          commentId: currentPostORComment._id
        })
      );
    }
    else {
      await dispatch(
        createAPostCommentAction({
          replyInfo: data,
          postId: currentPostORComment._id
        })
      );
    }
  ...
  const { owner, text, meta } = currentPostORComment

  const baseCommentInReply = () => {
    return <ShowBaseCommentInReply imgUrl={"https://images.unsplash.com/placeholder-avatars/extra-large.jpg"} timeAgo={TimeAgoFormat(meta.createdAt)} message={text} firstname={owner.firstName} lastname={owner.lastName} />;
  };
  ...
  return (
    <Modal isOpen={isModalOpen} closeModal={closeModal} title={title}>
      <div className="w-[306px] md:w-full md:mb-6 overflow-x-hidden overflow-y-auto">
        ...
        (
          <div className="my-2">
            {(comments || []).map((comments: any, index: any) => <CommentHolder key={index} replies={comments} />)}
          </div>
        )
        } */}
